...
function generateLog($info){
	$today = date("d-m-Y");
	$fpath = "/home/wecarysa/public_html/prasad_1/log_info/".$today.".txt";
	if(!fopen($fpath, 'r')){
...
function showHeader($page){
	...
	if($wc_uid){
		...
		echo '          <ul class="dropdown-menu">
			                <li><a href="myAddress.php">Address</a></li>
...
function showFooter(){
	...
	echo '      <p>&copy; 2015 WeCarriers. &middot; <a href="privacy.php"><button class="btn btn-link">Privacy</button></a> &middot; <a href="terms.php"><button class="btn btn-link">Terms</button></a></p>';
	echo '    </footer>';
}

function showSidebarTop($page){

	echo '    <div class="col-sm-3 col-lg-2 col-md-2 col-xs-12 sidebar">';
	      		...
	      		<div class="modal-body">
	      			<div class="text-center">
					  	...
					  	<ul class="nav nav-tabs" role="tablist" style="width:100%;">
					    	...
					    	<li role="presentation"><a href="#forget" aria-controls="forget" role="tab" data-toggle="tab"><span style="margin-right:5px;"><i class="fa fa-lock"></i></span>Forgot Password</a></li>
					  	</ul>
						            ...
						            <div class="row">
						            	<div class="col-md-12">
							              <div class="input-group" style="padding-bottom: 10px;">
							                <div class="input-group-addon"><i class="fa fa-fw fa-mobile"></i></div>							                
		...
		<div class="container">
		   <div id="'.$link_id.'" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
					   ...
					   echo '<div class="col-md-6">';
						echo '<div data-dismiss="modal" class="panel panel-primary" style=" cursor: pointer; overflow: auto;" onclick="addAddressToField(\''.$row['address_id'].'\', \''.$row['address_label'].'\', \''.$input_id.'\')">
							...
							<div class="panel-body">
								...
								<div style=""><label style="width: 30%;font-weight: bold;">City</label><label style="font-weight: normal;"></label>'.$row['city'].'</div>
								<div style=""><label style="width: 30%;font-weight: bold;">State</label><label style="font-weight: normal;"></label>'.$row['state'].'</div>
			...
			echo '		</div>
					...
					<div style="text-align:center"><a href="myAddress.php"><button type="button" class="btn btn-link">Add New Address</button></a></div>
				</div>
			</div>
		      </div>
		   </div>
				...
				if($res && mysqli_num_rows($res) > 0){
					...
					while($row = mysqli_fetch_array($res)){
						...
						echo '<div style="" class="col-md-6">
							<div data-dismiss="modal" class="panel panel-primary" style=" cursor: pointer; overflow: auto;" onclick="addJourneyToField(\''.$row['journey_id'].'\', \''.$row['headline'].'\',\''.$row['notes'].'\',\''.$row['journey_date'].'\',\''.$row1['address'].'\',\''.$row1['city'].'\',\''.$row1['state'].'\',\''.$row1['pincode'].'\',\''.$row2['address'].'\',\''.$row2['city'].'\',\''.$row2['state'].'\',\''.$row2['pincode'].'\')">
							...
							echo '
								...
								<div style="">';
									...
									<label style="font-weight: normal;">'.$row2['city'].'</label>, 
									<label style="font-weight: normal;">'.$row2['state'].'</label>, 
									<label style="font-weight: normal;">India</label>,
			   ...
			   <div style="">';
				...
				if($res && mysqli_num_rows($res) > 0){
					...
					while($row = mysqli_fetch_array($res)){
								...
								$row2 = mysqli_fetch_array($res2);
						echo '<div style="" class="col-md-6">
							<div data-dismiss="modal" class="panel panel-primary" style=" cursor: pointer; overflow: auto;" onclick="addOrderToField(\''.$row['order_id'].'\', \''.$row['headline'].'\',\''.$row['price'].'\',\''.$row['journey_date'].'\',\''.$row1['address'].'\',\''.$row1['city'].'\',\''.$row1['state'].'\',\''.$row1['pincode'].'\',\''.$row2['address'].'\',\''.$row2['city'].'\',\''.$row2['state'].'\',\''.$row2['pincode'].'\')">
						  	<div class="panel-heading" style="text-align: center;">
						  		<h3 class="panel-title">'.$row['headline'].'</h3>
						  	</div>
							<div class="panel-body">
